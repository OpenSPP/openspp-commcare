...
class XFormToOdoo(models.TransientModel):
    ...
    def create_odoo_model(self, xform):
        try:
            ...
            raise exceptions.ValidationError(f"An error occurred: {str(e)}") from e
    ...
    def create_field(self, model_id, field_name, field_attrs):
        ...
        if not field_type:
            _logger.info(f"Field type not found for {field_display_name}")
            return
        ...
        if select_options:
            for option in select_options:
                value = option["value"]
                display_name = option["label"].get(
                    "en", value
                )  # Default to the value if 'en' label is not available
    ...
    def _parse_xml(self, xform):
        ...
        instance_element = model_element.xpath(
            ".//xf:instance", namespaces=self._xform_namespaces
        )[0]
    ...
    def _update_json_structure(self, element, json_dict, field_types):
        for child in element:
            _logger.info("child.tag: ", child.tag)
            _logger.info("child.attrib: ", child.attrib)
            tag = child.tag.split("}")[-1]
            field_info = {
            ...
            if child.attrib.get("{http://openrosa.org/javarosa}template") is not None:
                _logger.info("jr:template found")
                json_dict[tag] = {
                    "type": "list",
                    "_model_name": tag,
                ...
                json_dict[tag] = self._update_json_structure(
                    child, field_info, field_types
                )
        return json_dict
    ...
    def _extract_translations(self, model_element):
        translations = {}
        for itext in model_element.xpath(
            ".//xf:itext/xf:translation", namespaces=self._xform_namespaces
        ):
            lang = itext.get("lang")
            for text in itext.xpath(".//xf:text", namespaces=self._xform_namespaces):
                field_id = text.get("id")
                label = text.xpath(
                    "./xf:value/text()", namespaces=self._xform_namespaces
                )
    ...
    def _update_json_with_translations(
        self, json_dict, translations, current_path=None
    ):
        for key, value in json_dict.items():
            ...
            field_id = (
                "/".join(new_path) + "-label"
            )  # Use forward slash (/) instead of hyphen (-)
            ...
            if isinstance(value, dict):
                if "select_options" in value and value["select_options"] is not None:
                    for option in value["select_options"]:
                        ...
                        if option_value:
                            option_id = (
                                "/".join(new_path) + "-" + option_value + "-label"
                            )  # Use forward slash (/) instead of hyphen (-)
    ...
    def xform_to_dict(self, xform):
        ...
        for bind in model_element.xpath(
            ".//xf:bind", namespaces=self._xform_namespaces
        ):
        ...
        json_structure = self._update_json_structure(
            data_element, json_structure, field_types
        )

        for select_element in body_element.xpath(
            ".//xf:select1", namespaces=self._xform_namespaces
        ):
